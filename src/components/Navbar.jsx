...
function Navbar() {
  ...
  return (
    <motion.div 
    className={`fixed left-0 top-0 backdrop-blur-md z-[999] w-full lg:py-5 lg:px-11 py-5 px-4 flex lg:justify-between items-end font-['Neue_Montreal'] transition-all`}
    initial={{y: 0}}
    animate={{y: isScrollingUp? "-80px":"0px"}}
    transition={{duration: 0.3, ease: "easeInOut"}}
    >

      <div className="logo mb-1 mr-4">
      <svg
        className="lg:w-20 lg:h-7 w-12 h-5"
        viewBox="0 0 72 30"
        fill="none"
      ...
      <div className="links flex lg:items-center items-end lg:gap-4 gap-1">
          {navTexts.map((link,index)=>(
            <a key={index} className={`min-w-16 lg:text-[1.25vw] text-[3.8vw] capitalize ${index === 4 ? "ml-48" : ""}`} href="#">{link}</a>
          ))}
      </div>
    </motion.div>
  );
